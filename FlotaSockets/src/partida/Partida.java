...
public class Partida {
	 ...
	 */	
    public String pruebaCasilla(int f, int c) {

    	this.disparos++;
    	
    	/** Si es agua, tocado o hundido devolvemos el estado en el que se encuentra. Saco los datos del barco
    	 * y dependiendo de la orientación se cambian los valores en una dirección o en otra en la matriz*/
    	int estado = mar[f][c];
    	if(mar[f][c] == AGUA || mar[f][c] == TOCADO || mar[f][c] == HUNDIDO){
			return Integer.toString(estado);
    	}
    	
    	String barco [] = getBarco(mar[f][c]).split("#");
		...
		Barco boat = barcos.get(estado);
		
		if(boat.tocaBarco()){
			if(orient == 'H'){
    			for (int i = 0; i < tam; i++){
    				mar[filIni][colIni + i] = HUNDIDO;
    			}
			}else{
				for (int i = 0; i < tam; i++){
    				mar[filIni + i][colIni] = HUNDIDO;
    			}
			}
			return Integer.toString(estado);
		}else{
			estado = mar[f][c] = TOCADO;
			return Integer.toString(estado);
		}
	...
	public String[] getSolucion() {
		...
		String sol [] = new String [barcos.size()];
